...
| 缺点  | 隐藏依赖关系、不能设为不可变(final) | 依赖越多构造函数参数越长              | 依赖关系不明显、依赖可能缺失        |
# 两种注入注解
---
...
- 优先按类型在容器中查找：
	...
	2. 找到一个：直接注入。
	3. 找到多个：逐步执行直到选出Bean。
		1. 检查是否存在`@Qualifier`标记的Bean，并匹配变量或参数名。
		2. 检查是否存在`@Primary`标记的Bean，有则优先注入。
		3. 检查是否有`@Priority`标记。
...
3. **填充**：实例化后，`AutowiredAnnotationBeanPostProcessor`的前置处理会扫描bean的字段和方法，扫描到`@Autowired`就利用反射对字段进行赋值或利用反射调用Setter方法进行赋值。
